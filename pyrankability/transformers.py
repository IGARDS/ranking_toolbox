    ...
    def transform(self, X, y = None ):
        ...
        for days_to_subtract_key,year in tqdm(itertools.product(self.days_to_subtract_keys,self.years)):
            ...
            for index,row in best_df.iterrows():
                ...
                if iw == 0:
                    st = np.Inf
                pipe = Pipeline([('compute_D', ComputeDTransformer(team_range, dt, st))])
        ...
        for index,row in tqdm(X.iterrows()):
            ...
            for i,D in enumerate(X.loc[(year,days_to_subtract_key,dt,st,iw,ran,method),"D"]):
                ...
                if i == 0:
                    ...
                    raise Exception("Error")
                features = utils.compute_features(D, rankings, self.top_k, self.feature_columns)
                features.name = tuple(list(index)+[construction])
                X_new = X_new.append(features)

                if i == 1:
                    construction = "Both"
                    features = utils.compute_features(sum_D, rankings, self.top_k, self.feature_columns)
                    features.name = tuple(list(index)+[construction])
                    X_new = X_new.append(features)
        return X_new
...
class IndividualGamesCountTransformer( BaseEstimator, TransformerMixin ):
    ...
    def transform(self, games , y = None ):
        ...
        map_func = lambda linked: construct.support_map_vectorized_direct_indirect(linked,direct_thres=self.direct_thres,spread_thres=self.spread_thres)
        Ddirect,Dindirect = construct.V_count_vectorized(games,map_func)
